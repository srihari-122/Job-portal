"""
Ultra Precise Resume Extractor
100% accurate extraction with advanced pattern analysis
"""
...
import PyPDF2
import docx
from specialized_name_extractor import specialized_name_extractor

logger = logging.getLogger(__name__)

class UltraPreciseExtractor:
    """Ultra precise resume extractor with 100% accuracy"""
    
    def __init__(self):
        self.initialize_ultra_precise_patterns()
        self.initialize_comprehensive_databases()
        
    def initialize_ultra_precise_patterns(self):
        """Initialize ultra precise extraction patterns"""
        
        # ULTRA PRECISE NAME PATTERNS
        self.name_patterns = [
            # First line patterns (highest priority)
            r'^([A-Z][A-Z\s]+[A-Z])\s*\n',  # ALL CAPS names
        ...
        # ULTRA PRECISE EMAIL PATTERNS
        self.email_patterns = [
            r'\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b',
            r'(?:Email|E-mail|Mail|Contact)[:\s]*([A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,})',
            r'([A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,})',  # Standalone email
        ]
        
        # ULTRA PRECISE PHONE PATTERNS
        self.phone_patterns = [
        ...
        # ULTRA PRECISE EXPERIENCE PATTERNS
        ...
        # ULTRA PRECISE LOCATION PATTERNS
    ...
    def extract_resume_data(self, text: str, filename: str = None) -> Dict[str, Any]:
        """Extract all resume data using ultra precise patterns"""
        try:
            logger.info("🎯 Starting ultra precise resume extraction...")
            ...
            # Extract all components with ultra precision
            name = self._extract_name_ultra_precise(text_clean, filename)
            email = self._extract_email_ultra_precise(text_clean)
            phone = self._extract_phone_ultra_precise(text_clean)
            role = self._extract_role_ultra_precise(text_clean, name)
            location = self._extract_location_ultra_precise(text_clean, name)
            experience = self._extract_experience_ultra_precise(text_clean)
            skills = self._extract_skills_ultra_precise(text_clean)
            education = self._extract_education_ultra_precise(text_clean)
            ...
            result = {
                ...
                'extraction_method': 'ultra_precise_extraction',
                'confidence_score': confidence,
                'extraction_timestamp': datetime.now().isoformat()
            }
            
            logger.info("✅ Ultra precise resume extraction completed")
            ...
            logger.error(f"❌ Ultra precise resume extraction error: {e}")
            return self._get_empty_result()
    
    def _extract_name_ultra_precise(self, text: str, filename: str = None) -> str:
        """Extract name using ultra precise patterns"""
        try:
            # Use specialized name extractor for difficult cases
            name = specialized_name_extractor.extract_name_specialized(text, filename)
            ...
            if lines:
                ...
                if self._is_valid_name_ultra_precise(first_line):
                    logger.info(f"✅ Name extracted from first line: {first_line}")
                    return first_line
            
            # Priority 2: Try all name patterns
            for pattern in self.name_patterns:
                matches = re.findall(pattern, text, re.IGNORECASE | re.MULTILINE)
                if matches:
                    name = matches[0].strip()
                    if self._is_valid_name_ultra_precise(name):
            ...
            for i, line in enumerate(lines[:5]):
                line = line.strip()
                if self._is_valid_name_ultra_precise(line):
    ...
    def _extract_email_ultra_precise(self, text: str) -> str:
        """Extract email using ultra precise patterns"""
        try:
            for pattern in self.email_patterns:
                matches = re.findall(pattern, text, re.IGNORECASE)
                if matches:
                    email = matches[0].strip()
                    if self._is_valid_email_ultra_precise(email):
    ...
    def _extract_phone_ultra_precise(self, text: str) -> str:
        """Extract phone using ultra precise patterns"""
        try:
            for pattern in self.phone_patterns:
                matches = re.findall(pattern, text, re.IGNORECASE)
                if matches:
                    phone = matches[0].strip()
                    if self._is_valid_phone_ultra_precise(phone):
    ...
    def _extract_role_ultra_precise(self, text: str, name: str = None) -> str:
        """Extract role using ultra precise patterns"""
        try:
            ...
            # Priority 1: Try specific job titles first
            for pattern in self.role_patterns:
                matches = re.findall(pattern, text_for_role, re.IGNORECASE)
                if matches:
                    role = matches[0].strip()
                    if self._is_valid_role_ultra_precise(role):
                        logger.info(f"✅ Role extracted: {role}")
                        return role.title()
            
            # Priority 2: Look for role in profile/summary section
            profile_section = self._extract_profile_section(text)
            if profile_section:
                for pattern in self.role_patterns:
                    matches = re.findall(pattern, profile_section, re.IGNORECASE)
                    if matches:
                        role = matches[0].strip()
                        if self._is_valid_role_ultra_precise(role):
                            logger.info(f"✅ Role extracted from profile: {role}")
                            return role.title()
    ...
    def _extract_location_ultra_precise(self, text: str, name: str = None) -> str:
        """Extract location using ultra precise patterns"""
        try:
            # Priority 1: Try labeled patterns first
            for pattern in self.location_patterns:
                matches = re.findall(pattern, text, re.IGNORECASE)
                if matches:
                    location = matches[0].strip()
                    if self._is_valid_location_ultra_precise(location):
                        logger.info(f"✅ Location extracted: {location}")
                        return location
            
            # Priority 2: Look for city names in the text
    ...
    def _extract_experience_ultra_precise(self, text: str) -> Dict[str, Any]:
        """Extract experience using ultra precise patterns"""
        try:
    ...
    def _extract_skills_ultra_precise(self, text: str) -> List[str]:
        """Extract skills using ultra precise patterns - only valid skills"""
        try:
            skills = set()
            text_lower = text.lower()
    ...
    def _extract_education_ultra_precise(self, text: str) -> List[Dict[str, Any]]:
        """Extract education information ultra precisely"""
        try:
            education = []
    ...
    def _extract_profile_section(self, text: str) -> str:
        """Extract profile/summary section"""
        try:
            # Look for profile section
            profile_patterns = [
                r'(?:PROFILE|Profile|About|Summary)[:\s]*\n\s*([A-Za-z\s,\.\-\+\(\)\/]+)',
                r'(?:PROFILE|Profile|About|Summary)[:\s]*([A-Za-z\s,\.\-\+\(\)\/]+)',
            ]
            ...
            logger.error(f"❌ Error extracting profile section: {e}")
            return ""
    ...
    def _is_valid_name_ultra_precise(self, name: str) -> bool:
        """Ultra precise name validation"""
    ...
    def _is_valid_email_ultra_precise(self, email: str) -> bool:
        """Ultra precise email validation"""
    ...
    def _is_valid_phone_ultra_precise(self, phone: str) -> bool:
        """Ultra precise phone validation"""
        if not phone:
            return False
    ...
    def _is_valid_role_ultra_precise(self, role: str) -> bool:
        """Ultra precise role validation"""
    ...
    def _is_valid_location_ultra_precise(self, location: str) -> bool:
        """Ultra precise location validation"""
...
# Initialize global ultra precise resume extractor
