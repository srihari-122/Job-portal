"""
Specialized Name Extractor
Handles specific name extraction cases with advanced pattern matching
"""
...
class SpecializedNameExtractor:
    """Specialized name extractor for difficult cases"""
    
    def __init__(self):
        self.initialize_specialized_patterns()
        
    def initialize_specialized_patterns(self):
        """Initialize specialized name patterns"""
        
        # Specialized name patterns for difficult cases
    ...
    def extract_name_specialized(self, text: str, filename: str = None) -> str:
        """Extract name using specialized patterns"""
        try:
            logger.info("🔍 Starting specialized name extraction...")
            ...
            if lines:
                ...
                if self._is_valid_name_specialized(first_line):
                    logger.info(f"✅ Name extracted from first line: {first_line}")
                    return first_line
            
            # Priority 2: Try all specialized patterns
            for pattern in self.specialized_patterns:
                matches = re.findall(pattern, text, re.IGNORECASE | re.MULTILINE)
                if matches:
                    name = matches[0].strip()
                    if self._is_valid_name_specialized(name):
                        logger.info(f"✅ Name extracted with specialized pattern: {name}")
                        return name
            
            # Priority 3: Look for name in first few lines
            for i, line in enumerate(lines[:5]):
                line = line.strip()
                if self._is_valid_name_specialized(line):
            ...
            logger.error(f"❌ Specialized name extraction error: {e}")
            return 'Name not found'
    
    def _is_valid_name_specialized(self, name: str) -> bool:
...
# Initialize global specialized name extractor
specialized_name_extractor = SpecializedNameExtractor()
