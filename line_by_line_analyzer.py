...
Scans through the entire resume line by line to find the actual name
"""
...
class LineByLineAnalyzer:
    """Line-by-line analyzer that scans through the entire resume"""
    
    def __init__(self):
        self.initialize_line_patterns()
        self.initialize_comprehensive_databases()
        
    def initialize_line_patterns(self):
        """Initialize line-by-line extraction patterns"""
        
        # LINE-BY-LINE NAME PATTERNS
        self.name_patterns = [
            # ALL CAPS names
            r'^([A-Z][A-Z\s]+[A-Z])$',
            r'^([A-Z][A-Z]+ [A-Z][A-Z]+)$',
        ...
        # LINE-BY-LINE EXPERIENCE PATTERNS
        ...
        # LINE-BY-LINE LOCATION PATTERNS
    ...
    def extract_resume_data(self, text: str, filename: str = None) -> Dict[str, Any]:
        """Extract all resume data using line-by-line analysis"""
        try:
            logger.info("🎯 Starting line-by-line analysis...")
            ...
            # Extract all components with line-by-line analysis
            name = self._extract_name_line_by_line(text_clean, filename)
            email = self._extract_email_line_by_line(text_clean)
            phone = self._extract_phone_line_by_line(text_clean)
            role = self._extract_role_line_by_line(text_clean, name)
            location = self._extract_location_line_by_line(text_clean, name)
            experience = self._extract_experience_line_by_line(text_clean)
            skills = self._extract_skills_line_by_line(text_clean)
            education = self._extract_education_line_by_line(text_clean)
            ...
            result = {
                ...
                'extraction_method': 'line_by_line_analysis',
                'confidence_score': confidence,
                'extraction_timestamp': datetime.now().isoformat()
            }
            
            logger.info("✅ Line-by-line analysis completed")
            ...
            logger.error(f"❌ Line-by-line analysis error: {e}")
            return self._get_empty_result()
    
    def _extract_name_line_by_line(self, text: str, filename: str = None) -> str:
        """Extract name by scanning line by line"""
        try:
            logger.info("🔍 Starting line-by-line name extraction...")
            
            lines = text.split('\n')
            
            # Scan through each line to find the name
            ...
            # If no name found in lines, try filename
            if filename:
            ...
            logger.error(f"❌ Line-by-line name extraction error: {e}")
            return 'Name not found'
    
    def _is_likely_name_line(self, line: str) -> bool:
        """Check if a line is likely a name"""
    ...
    def _extract_email_line_by_line(self, text: str) -> str:
        """Extract email by scanning line by line"""
        try:
            for pattern in self.email_patterns:
                matches = re.findall(pattern, text, re.IGNORECASE)
                if matches:
                    email = matches[0].strip()
                    if self._is_valid_email_line_by_line(email):
            ...
            logger.error(f"❌ Line-by-line email extraction error: {e}")
            return 'Email not found'
    
    def _extract_phone_line_by_line(self, text: str) -> str:
        """Extract phone by scanning line by line"""
        try:
            for pattern in self.phone_patterns:
                matches = re.findall(pattern, text, re.IGNORECASE)
                if matches:
                    phone = matches[0].strip()
                    if self._is_valid_phone_line_by_line(phone):
            ...
            logger.error(f"❌ Line-by-line phone extraction error: {e}")
            return 'Phone not found'
    
    def _extract_role_line_by_line(self, text: str, name: str = None) -> str:
        """Extract role by scanning line by line"""
        try:
            ...
            for pattern in self.role_patterns:
                ...
                if matches:
                    ...
                    if self._is_valid_role_line_by_line(role):
            ...
            logger.error(f"❌ Line-by-line role extraction error: {e}")
            return 'Role not found'
    
    def _extract_location_line_by_line(self, text: str, name: str = None) -> str:
        """Extract location by scanning line by line"""
        try:
            ...
            for pattern in self.location_patterns:
                ...
                if matches:
                    ...
                    if self._is_valid_location_line_by_line(location):
            ...
            logger.error(f"❌ Line-by-line location extraction error: {e}")
            return 'Location not found'
    
    def _extract_experience_line_by_line(self, text: str) -> Dict[str, Any]:
        """Extract experience by scanning line by line"""
        try:
            ...
            logger.error(f"❌ Line-by-line experience extraction error: {e}")
            return {
                'total_years': 0,
                'total_months': 0,
    ...
    def _extract_skills_line_by_line(self, text: str) -> List[str]:
        """Extract skills by scanning line by line"""
        try:
            skills = set()
            text_lower = text.lower()
            ...
            logger.error(f"❌ Line-by-line skills extraction error: {e}")
            return []
    
    def _extract_education_line_by_line(self, text: str) -> List[Dict[str, Any]]:
        """Extract education by scanning line by line"""
        try:
            education = []
            ...
            logger.error(f"❌ Line-by-line education extraction error: {e}")
            return []
    ...
    def _is_valid_email_line_by_line(self, email: str) -> bool:
        """Line-by-line email validation"""
    ...
    def _is_valid_phone_line_by_line(self, phone: str) -> bool:
        """Line-by-line phone validation"""
        if not phone:
            return False
    ...
    def _is_valid_role_line_by_line(self, role: str) -> bool:
        """Line-by-line role validation"""
    ...
    def _is_valid_location_line_by_line(self, location: str) -> bool:
        """Line-by-line location validation"""
...
# Initialize global line-by-line analyzer
line_by_line_analyzer = LineByLineAnalyzer()
