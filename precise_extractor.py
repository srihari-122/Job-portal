...
Ultra-accurate extraction using advanced pattern matching and validation
"""
...
class PreciseResumeExtractor:
    """Ultra-precise resume extractor with advanced validation"""
    
    def __init__(self):
        self.skill_database = self._load_comprehensive_skills()
        self.location_database = self._load_locations()
        self.role_keywords = self._load_role_keywords()
        
    def _load_comprehensive_skills(self) -> List[str]:
    ...
    def _load_locations(self) -> List[str]:
        ...
        return [
            ...
            'mandya', 'hassan', 'udupi', 'dakshina kannada', 'udupi', 'chikmagalur',
            'chitradurga', 'davangere', 'shimoga', 'chamrajanagar', 'kodagu', 'mysuru'
        ]
    
    def _load_role_keywords(self) -> List[str]:
        """Load role keywords"""
        return [
            'developer', 'engineer', 'programmer', 'architect', 'consultant', 'manager',
            'analyst', 'scientist', 'designer', 'administrator', 'specialist', 'lead',
            'senior', 'junior', 'principal', 'staff', 'director', 'head', 'vp', 'cto',
            'full stack', 'frontend', 'backend', 'mobile', 'devops', 'data', 'machine learning',
            'ai', 'software', 'web', 'cloud', 'security', 'qa', 'test', 'ui', 'ux',
            'product', 'business', 'system', 'network', 'database', 'sales', 'marketing',
            'hr', 'finance', 'operations', 'project', 'scrum master', 'technical lead'
        ]
    
    def extract_resume_content(self, text: str, filename: str = None) -> Dict[str, Any]:
        """Extract resume content with ultra-precision"""
        try:
            ...
            logger.info(f"✅ Precise extraction completed:")
            ...
            logger.error(f"❌ Error in precise resume extraction: {e}")
            return self._get_empty_extraction()
    ...
    def _extract_name_precise(self, text: str, filename: str = None) -> str:
        ...
        try:
            ...
            # Strategy 2: Look for name patterns in headers
            lines = text.split('\n')
    ...
    def _extract_role_precise(self, text: str, name: str) -> str:
        """Extract role with precision, excluding names"""
        try:
            ...
            # Look for role in work experience section first
            ...
            for pattern in work_section_patterns:
                ...
                for match in matches:
                    ...
                    if len(role.split()) >= 2:
                        ...
                        if not role_words.intersection(name_words):
                            return role.title()
            ...
            for pattern in role_patterns:
                matches = re.findall(pattern, text, re.IGNORECASE)
                for match in matches:
                    # Check if match contains name words
                    match_words = set(match.lower().split())
                    if not match_words.intersection(name_words):
                        return match.title()
                ...
                if any(indicator in line_lower for indicator in role_indicators):
                    ...
                    if 2 <= len(words) <= 6:
                        ...
                        if words[0][0].isupper():
                            ...
                            if not line_words.intersection(name_words):
                                return line.title()
    ...
    def _extract_location_precise(self, text: str, name: str) -> str:
        """Extract location with precision, excluding names"""
        try:
            # Get name words to exclude
            name_words = set()
            if name and name != 'Name not found':
            ...
            for pattern in location_patterns:
                ...
                if match:
                    ...
                    if len(location.split()) >= 1:
                        ...
                        if not location_words.intersection(name_words):
                            return location.title()
            
            # Look for known locations in text (excluding names)
            text_lower = text.lower()
            for location in self.location_database:
                if location in text_lower and location not in name_words:
                    return location.title()
            ...
            for pattern in contact_sections:
                ...
                for match in matches:
                    ...
                    for location in self.location_database:
                        if location in section_text and location not in name_words:
                            return location.title()
    ...
    def _extract_skills_precise(self, text: str) -> List[str]:
        """Extract skills with precision - only from resume content"""
        try:
            skills = []
            text_lower = text.lower()
...
# Initialize global precise extractor instance
precise_extractor = PreciseResumeExtractor()
