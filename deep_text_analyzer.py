"""
Deep Text Analyzer
Handles resumes where all content is in one long line
"""
...
class DeepTextAnalyzer:
    """Deep text analyzer that handles complex resume formats"""
    
    def __init__(self):
        self.initialize_deep_patterns()
        self.initialize_comprehensive_databases()
        
    def initialize_deep_patterns(self):
        """Initialize deep extraction patterns"""
        
        # DEEP NAME PATTERNS - Extract from within long text
        self.name_patterns = [
            # Look for names within the text (not just at the beginning)
            r'\b([A-Z][A-Z]+ [A-Z][A-Z]+)\b',  # ALL CAPS names
            r'\b([A-Z][a-z]+ [A-Z][a-z]+(?: [A-Z][a-z]+)?)\b',  # Title Case names
            r'\b([A-Z][a-z]+ [A-Z]\. [A-Z][a-z]+)\b',  # First M. Last
            r'\b([A-Z][A-Z]+ [A-Z]\. [A-Z][A-Z]+)\b',  # ALL CAPS with middle initial
            
            # Look for names before common sections
            r'([A-Z][A-Z]+ [A-Z][A-Z]+)\s+(?:PROFILE|Profile|About|Summary|SKILLS|Skills|EDUCATION|Education)',
            r'([A-Z][a-z]+ [A-Z][a-z]+(?: [A-Z][a-z]+)?)\s+(?:PROFILE|Profile|About|Summary|SKILLS|Skills|EDUCATION|Education)',
        ]
    ...
    def extract_resume_data(self, text: str, filename: str = None) -> Dict[str, Any]:
        """Extract all resume data using deep text analysis"""
        try:
            logger.info("🎯 Starting deep text analysis...")
            ...
            # Extract all components with deep analysis
            name = self._extract_name_deep(text_clean, filename)
            email = self._extract_email_deep(text_clean)
            phone = self._extract_phone_deep(text_clean)
            role = self._extract_role_deep(text_clean, name)
            location = self._extract_location_deep(text_clean, name)
            experience = self._extract_experience_deep(text_clean)
            skills = self._extract_skills_deep(text_clean)
            education = self._extract_education_deep(text_clean)
            ...
            result = {
                ...
                'extraction_method': 'deep_text_analysis',
                'confidence_score': confidence,
                'extraction_timestamp': datetime.now().isoformat()
            }
            
            logger.info("✅ Deep text analysis completed")
            ...
            logger.error(f"❌ Deep text analysis error: {e}")
            return self._get_empty_result()
    
    def _extract_name_deep(self, text: str, filename: str = None) -> str:
        """Extract name using deep text analysis"""
        try:
            logger.info("🔍 Starting deep name extraction...")
            
            # First, try to find the name in the filename
            if filename:
            ...
            # If filename doesn't work, look for names within the text
            # Look for names that appear before common resume sections
            for pattern in self.name_patterns:
                matches = re.findall(pattern, text, re.IGNORECASE)
                if matches:
                    for match in matches:
                        name = match.strip()
                        if self._is_likely_name_deep(name):
                            logger.info(f"✅ Name found with deep pattern: {name}")
                            return name
            
            # If still no name, look for any valid name pattern in the text
            lines = text.split('\n')
            for i, line in enumerate(lines):
                line = line.strip()
                if self._is_likely_name_deep(line):
            ...
            logger.error(f"❌ Deep name extraction error: {e}")
            return 'Name not found'
    
    def _is_likely_name_deep(self, text: str) -> bool:
        """Check if text is likely a name using deep analysis"""
    ...
    def _extract_email_deep(self, text: str) -> str:
        """Extract email using deep analysis"""
        try:
            for pattern in self.email_patterns:
                matches = re.findall(pattern, text, re.IGNORECASE)
                if matches:
                    email = matches[0].strip()
                    if self._is_valid_email_deep(email):
            ...
            logger.error(f"❌ Deep email extraction error: {e}")
            return 'Email not found'
    
    def _extract_phone_deep(self, text: str) -> str:
        """Extract phone using deep analysis"""
        try:
            for pattern in self.phone_patterns:
                matches = re.findall(pattern, text, re.IGNORECASE)
                if matches:
                    phone = matches[0].strip()
                    if self._is_valid_phone_deep(phone):
            ...
            logger.error(f"❌ Deep phone extraction error: {e}")
            return 'Phone not found'
    
    def _extract_role_deep(self, text: str, name: str = None) -> str:
        """Extract role using deep analysis"""
        try:
            ...
            for pattern in self.role_patterns:
                ...
                if matches:
                    ...
                    if self._is_valid_role_deep(role):
            ...
            logger.error(f"❌ Deep role extraction error: {e}")
            return 'Role not found'
    
    def _extract_location_deep(self, text: str, name: str = None) -> str:
        """Extract location using deep analysis"""
        try:
            ...
            for pattern in self.location_patterns:
                ...
                if matches:
                    ...
                    if self._is_valid_location_deep(location):
            ...
            logger.error(f"❌ Deep location extraction error: {e}")
            return 'Location not found'
    
    def _extract_experience_deep(self, text: str) -> Dict[str, Any]:
        """Extract experience using deep analysis"""
        try:
            text_lower = text.lower()
            ...
            logger.error(f"❌ Deep experience extraction error: {e}")
            return {
                'total_years': 0,
                'total_months': 0,
    ...
    def _extract_skills_deep(self, text: str) -> List[str]:
        """Extract skills using deep analysis"""
        try:
            skills = set()
            text_lower = text.lower()
            ...
            logger.error(f"❌ Deep skills extraction error: {e}")
            return []
    
    def _extract_education_deep(self, text: str) -> List[Dict[str, Any]]:
        """Extract education using deep analysis"""
        try:
            education = []
            ...
            logger.error(f"❌ Deep education extraction error: {e}")
            return []
    ...
    def _is_valid_email_deep(self, email: str) -> bool:
    ...
    def _is_valid_phone_deep(self, phone: str) -> bool:
        """Deep phone validation"""
        if not phone:
            return False
    ...
    def _is_valid_role_deep(self, role: str) -> bool:
        """Deep role validation"""
    ...
    def _is_valid_location_deep(self, location: str) -> bool:
        """Deep location validation"""
...
# Initialize global deep text analyzer
deep_text_analyzer = DeepTextAnalyzer()
